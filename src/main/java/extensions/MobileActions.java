...
public class MobileActions extends CommonOps{
    ...
    @Step("Drag and drop")
    public static void dragAndDrop(MobileElement startElem, MobileElement finishElem) {
        TouchAction action = new TouchAction(mobileDriver);
        action.press(new ElementOption()
                .withElement(startElem))
                .moveTo(new ElementOption().withElement(finishElem))
                .release()
                .perform();
    }
